...
public class CommentGlobalExceptionHandlerTest {
    ...
    @Test
    public void commentHandleExceptionNotFoundCommentByID() throws Exception {
        when(commentRESTController.getCommentByID(anyLong())).thenThrow(new NoSuchCommentException("Unexpected Exception"));

        mockMvc.perform(get("/api/v1/comments/{id}", anyLong()))
